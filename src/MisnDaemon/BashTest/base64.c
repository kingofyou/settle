...
{
    fprintf( stderr, "Usage: %s [-e/d] [input_filename] [output_filepath]\n", filepath );
    fprintf( stderr, "\t-d\tdecode infilepath\n\n" );
    fprintf( stderr, "\t-e\tencode infilepath\n\n" );
}

int main(int argc, char * argv[])
{
    
    char bindata[2050]="MjAxNTEyMDQwMDAxMnx8NjIyMjgwMTIzNDU2Nzg4ODk1M3y0+ri2uMTU7DAxfDEwNTEwMDAwMDAxN3wwfDEwMDAwLjAwfNeq1cvTw82+o7q0+reiuaTXynw=";
    char base64[4096];    
    base64_encode( bindata, base64, strlen(bindata));
    //printf( "%s\n", base64 );

    unsigned char bindata1[2050]="MjAxNTEyMDQwMDAxMnx8NjIyMjgwMTIzNDU2Nzg4ODk1M3y0+ri2uMTU7DAxfDEwNTEwMDAwMDAxN3wwfDEwMDAwLjAwfNeq1cvTw82+o7q0+reiuaTXynw=";
    char base641[4096];
    size_t bytes = base64_decode(  bindata1, base641);
    //printf( "%s\n", base641 );
    //printf( "%d\n", bytes );
    ...
    opterr = 0;
    while ( (opt = getopt(argc, argv, "ehdo:")) != -1 )
    {
        switch(opt)
        {
            case 'e':
    ...
    input_filename[sizeof(input_filename)-1] = '\0';
    strncpy(output_filename, argv[3], sizeof(output_filename));
