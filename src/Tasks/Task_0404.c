...
int Total_0404()
{
    ...
    getExtnFileFullName(&StExtnFileReg, sFile_name);

    /* 取文件行数 */
    fp = fopen (sFile_name, "r");
    if (!fp)
    {
        HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "fopen %s error.", sFile_name);
        return -1;
    }
...
*函数返回∶ 0  -- 成功
            -1 -- 失败
***************************************************/
int Task_0404 ( int nBeginOffset, int nEndOffset )
{
    int		iRet = 0;
    int	    i  = 1, nTotCount=0;
    char    *pstr;
    ...
    while (i < nEndOffset + 1)
    {
        ...
        {
            sprintf(ht_tbl_wift_txn.wx_refund_order, "%8.8s111161511111", ht_tbl_wift_txn.trans_date);          
        }
        ...
        memset(sTmp, 0, sizeof(sTmp));        
        getValueByDelim(sFileRecord, sTmp, sDelim);//免充值券金额
        ...
        sprintf(ht_tbl_wift_txn.bus_tp, "%4.4s%4.4s", BT_BUS_TP_WECHAT, BT_SETT_TRAN_D);
    ...
    HtLog( gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "Task_0404 Succ .");
    return 0;
}
