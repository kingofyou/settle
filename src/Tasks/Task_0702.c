...
int Task_0702 ( int nBeginOffset, int nEndOffset )
{
    ...
    /*已过滤退款交易，后面单独处理，防止原支付交易未处理的情况下先把退款交易处理了*/
    ...
    while (1)
    {
        ...
        if (iRet)
        {
            if (iRet == DBS_NOTFOUND)
            {
                ...
                if(iCheckTag == 0)
                {
                    ...
                    // 支付网关多，待T2对账，T+1先入对账不平表，状态为疑似不平
                    ...
                    {
                        HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "两方对账-支付网关多,兴业银行无");
                ...
                if(memcmp(ht_tbl_gateway_txn.sett_date, dbtbl_date_inf.stoday, BT_DATE_LEN) < 0)
                {
                    ...
                    HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "三方对账-兴业无");
				    memcpy(tbl_err_flow.inter_brh_code, ext_inter_brh_code, sizeof(tbl_err_flow.inter_brh_code)-1);
					memcpy(tbl_err_flow.sett_date, dbtbl_date_inf.syesterday, sizeof(tbl_err_flow.sett_date)-1);
					memcpy(tbl_err_flow.plat_key, ht_tbl_gateway_txn.plat_key, sizeof(tbl_err_flow.plat_key)-1);
					memcpy(tbl_err_flow.flag_result, BT_FLAG_RESULT_PART_NULL, BT_FLAG_RESULT_LEN);
        ...
        if (iAmtTag == 1 && iPanTag == 1)
        {
            ...
            HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "三方对账支付网关和兴业银行对平，继续和虚拟核心对账");
        }
        else
        {            
            if(iCheckTag == 0)
			...
			if (iRet && iRet != DBS_NOTFOUND)
            {
                ...
                // 更新表HT_TBL_HOST_TXN、HT_TBL_GATEWAY_TXN、HT_TBL_CIB_TXN
        ...
        if (iRet)
        {
            if (iRet == DBS_NOTFOUND)
            {
                /*三方对账,虚拟核心无，兴业银行有，支付网关有*/
                HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "虚拟核心无");
    ...
    while (1)
    {
        ...
        if (iRet)
        {
            if (iRet == DBS_NOTFOUND)
            {
                ...
                else
                {
                    ...
                    RightTrim(ht_tbl_gateway_txn_org.inter_brh_code);
                    RightTrim(ht_tbl_gateway_txn_org.plat_key);
                    /*退款成功/失败&&原支付交易成功&&原支付交易未清算/第三方无&&兴业银行无(撤单)&&清算日期为T+1*/
                    ...
                    {
                        HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "退款成功&&原支付交易成功&&未清算&&兴业银行无(撤单)");
                    ...
                    HtLog (gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "原支付交易状态不为待清算等");
                }  
                ...
                {
                    /*两方对账-兴业疑似少*/
                    HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "两方对账-兴业疑似少");
                    memset(&ht_tbl_host_txn, 0, sizeof(ht_tbl_host_txn));
                    iRet = in_err_flow(BT_FLAG_RESULT_PART_ORLESS, &ht_tbl_cib_txn, &ht_tbl_host_txn, &ht_tbl_gateway_txn, 1);
                    if (iRet)
                    {
                ...
                if(memcmp(ht_tbl_gateway_txn.sett_date, dbtbl_date_inf.stoday, BT_DATE_LEN) < 0)
                {
                    HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "两方对账-支付网关多,兴业银行无,原支付交易无");
        ...
        {
            HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "交易状态失败[%s]", ht_tbl_gateway_txn.trans_state);
			// 更新疑似第三方少的不平数据为确定
			memcpy(tbl_err_flow.inter_brh_code, ext_inter_brh_code, sizeof(tbl_err_flow.inter_brh_code)-1);
			memcpy(tbl_err_flow.sett_date, dbtbl_date_inf.syesterday, sizeof(tbl_err_flow.sett_date)-1);
			memcpy(tbl_err_flow.plat_key, ht_tbl_gateway_txn.plat_key, sizeof(tbl_err_flow.plat_key)-1);
