...
*函数返回∶ 0  -- 成功
            -1 -- 失败
***************************************************/
int Task_0105 ( int nBeginOffset, int nEndOffset )
{
    int		iRet = 0;
    int     i = 0;
    char 	sFlag[1 + 1] = {0};
    ...
    /*快捷支付流水文件：商户号_交易日期(yyyymmdd)_回单类型(0/1)_文件生成时间(yyyymmddhhmmss).txt*/
    sprintf(StExtnFileReg.src_file_name, "%s_YYYYMMDD_0.txt",KeyInf.sMrchNo);
    
    pstr = strstr (StExtnFileReg.src_file_name, "YYYYMMDD");
    if (pstr)
        memcpy (pstr, dbtbl_date_inf.stoday, 8);

    getShareFileFullName(&StExtnFileReg, share_infile_name);
    ...
    while (1)
    {
        ...
        else if (iRet == GLOB_NOMATCH)
        {
			...
			if(memcmp("04", sCurMisnHr, 2) <= 0) {
				...
				sprintf(sms_content, "超时未获取到兴业银行快捷入账回单文件[%s]", share_infile_name);
    ...
    while (1)
    {
        ...
        else if (iRet == GLOB_NOMATCH)
        {
			...
			if(memcmp("04", sCurMisnHr, 2) <= 0) {
                ...
                // 发短信
				memset(sms_content, 0, sizeof(sms_content));
				sprintf(sms_content, "超时未获取到兴业银行快捷出账回单文件[%s]", share_outfile_name);
                nReturnCode = sendMobileMsg(sms_content);
				if(nReturnCode) {
                    HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "报警短信[%s]发送失败", sms_content);
					sleep(2);
        ...
        else
        {
             ...
             memset(sOut, 0, sizeof(sOut));
             GetFilename(share_outfile_name, sOut);
             strcat(cib_outfile_name, sOut);
             HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "本地对账文件:[%s]...", cib_outfile_name);
             break;    
        }
    }
