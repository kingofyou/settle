...
int Task_0103 ( int nBeginOffset, int nEndOffset )
{
	...
	strcpy(KeyInf.pay_way, "cib_alipay");
	...
	while(1) {
        ...
        /* 初始共享文件信息 */
		memset(alipay_file_name, 0, sizeof(alipay_file_name));
        ...
        iRet = getExtnFileFullName(&StExtnFileReg, alipay_file_name);
        if (iRet != 0)
        {
            HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "getExtnFileFullName: error src_file_name[%s]...", StExtnFileReg.src_file_name);
            return -1;
        ...
        HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "正在等待支付宝对账文件:[%s]...", alipay_file_name);
        ...
        while (1)
        {
		    ...
		    if(sharefilesize1 == sharefilesize2) {
		    	...
		    	sprintf(sCommand, "cp -f %s %s", share_file_name, alipay_file_name);
		        ...
		        /* 确认文件是否存在 */
                iRet = access(alipay_file_name, R_OK);
                if (iRet != 0)
                {
                    sleep(2);
                    continue;
                ...
                if(stat(alipay_file_name, &stat_buf_alipay) == -1)
                {
                    HtLog (gLogFile, HT_LOG_MODE_NORMAL, __FILE__, __LINE__, "sleep");
                    sleep(2);
                    continue;
		...
		while(1) {
            ...
            sprintf(sCmd, "cat %s >> %s", alipay_file_name, sFileName);
