...
*函数返回∶ 0  -- 成功
            -1 -- 失败
***************************************************/
int Task_0406 ( int nBeginOffset, int nEndOffset )
{
    int		iRet = 0;
   ...
   /* 初始文件信息 */
    ...
    while (i < nEndOffset + 1)
    {
        ...
        sprintf(ht_tbl_cib_txn.bus_tp,"%s%s", BT_BUS_TP_CIB6, BT_BUS_TP_CIB6);
    ...
    fclose (fp);
    HtLog( gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "Task_0406 Succ .");
    return 0;
}
/****************************** end of Task_0406.c ******************************/
