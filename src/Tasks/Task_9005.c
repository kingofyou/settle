...
*函数返回∶ 0  -- 成功
            -1 -- 失败
***************************************************/
int Task_9005 ( int nBeginOffset, int nEndOffset )
{
    int     nReturnCode;
    char    sDateTime[14+1];
    char    stlm_dt[8 + 1];
    ...
    memcpy(sEPayReq.sTcptType,"0",sizeof(sEPayReq.sTcptType)-1);      
    ...
    if(nReturnCode)
    {
        ...
        {
            HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, " 下载0-快捷入账回单settleFile error [%d]",nReturnCode);
            return -1;
        }
    }
    else
    {
         HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "下载0-快捷入账回单 settleFile succ [%d]",nReturnCode);           
    }
    ...
    HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "开始下载1-快捷出账回单文件...");     
    ...
    if(nReturnCode)
    {
        if(strstr(sEPayRsp.stEpayErr.sErrCode, "EPAY_10008"))
        {
           HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, " 下载1-快捷出账回单已经下载 [%s]",sEPayRsp.stEpayErr.sErrCode);              
        }
        else
        {
            HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, " 下载1-快捷出账回单settleFile error [%d]",nReturnCode);
            return -1;
        }
    }
    else
    {
         HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "下载1-快捷出账回单 settleFile succ [%d]",nReturnCode);           
    }
    ...
    HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "Task_9005 Succ .");   
    return 0;
}
