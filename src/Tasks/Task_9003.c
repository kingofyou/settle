...
////    strcpy(comconfig.key2,"CIBALIPAY");
...
//	strcpy(tbl_key_cert_inf.pay_way, "cib_alipay");
...
*函数返回∶ 0  -- 成功
            -1 -- 失败
***************************************************/
int Task_9003 ( int nBeginOffset, int nEndOffset )
{
    int     nReturnCode;
    char    sDateTime[14+1];
    char    stlm_dt[8 + 1];
	...
	while(1) {
        ...
        HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "开始支付宝商户号[%s]流水文件...",sEPayReq.sMchId); 
