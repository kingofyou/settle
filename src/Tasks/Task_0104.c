...
*函数返回∶ 0  -- 成功
            -1 -- 失败
***************************************************/
int Task_0104 ( int nBeginOffset, int nEndOffset )
{
    int		iRet = 0;
    char 	sFlag[1 + 1] = {0};
    char    *pstr;
    char	wechat_file_name[256] = {0};
	...
	while(1) {
        ...
        /* 初始共享文件信息 */
		memset(wechat_file_name, 0, sizeof(wechat_file_name));
        ...
        iRet = getExtnFileFullName(&StExtnFileReg, wechat_file_name);
        if (iRet != 0)
        {
            HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "getExtnFileFullName: error src_file_name[%s]...", StExtnFileReg.src_file_name);
            return -1;
        } 
        HtLog(gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, "正在等待微信对账文件:[%s]...", wechat_file_name);
       ...
       while (1)
        {
            ...
            if (iRet != 0)
            {
		    	...
		    	if(memcmp("04", sCurMisnHr, 2) <= 0) {
			    	...
			    	sprintf(sms_content, "超时未获取到微信对账文件[%s]", share_file_name);
        ...
        while (1)
        {
	    	...
	    	if(sharefilesize1 == sharefilesize2) {
		    	...
		    	sprintf(sCommand, "cp -f %s %s", share_file_name, wechat_file_name);
		        ...
		        /* 确认文件是否存在 */
                iRet = access(wechat_file_name, R_OK);
                if (iRet != 0)
                {
                    sleep(2);
                    continue;
                ...
                memset(&stat_buf_wechat, 0x00, sizeof(stat_buf_wechat));
                if(stat(wechat_file_name, &stat_buf_wechat) == -1)
                {
                    HtLog (gLogFile, HT_LOG_MODE_NORMAL, __FILE__, __LINE__, "sleep");
                    sleep(2);
                    continue;
                ...
                HtLog (gLogFile, HT_LOG_MODE_DEBUG, __FILE__, __LINE__, " fileSize[%d][%d]", stat_buf_share.st_size, stat_buf_wechat.st_size);
                if(sharefilesize1 != 0 && stat_buf_wechat.st_size == 0)
                {
                    HtLog (gLogFile, HT_LOG_MODE_NORMAL, __FILE__, __LINE__, " fileSize[%d][%d]", stat_buf_share.st_size, stat_buf_wechat.st_size);
                    sleep(2);
                    continue;
                }
            }
        ...
        while(1) {
            ...
            sprintf(sCmd, "cat %s >> %s", wechat_file_name, sFileName);
		...
		HtLog(gLogFile, HT_LOG_MODE_NORMAL, __FILE__, __LINE__, "微信商户[%s]对账文件已到达.", KeyInf.paying_merc_code);
    }

    HtLog(gLogFile, HT_LOG_MODE_NORMAL, __FILE__, __LINE__, "微信对账文件已到达.");
    return 0;
}
