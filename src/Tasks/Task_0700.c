...
*函数返回∶ 1
***************************************************/
static int in_err_flow(char *flag_result, tbl_alipay_wechat_def *ptbl_alipay_wechat, ht_tbl_host_txn_def *pht_tbl_host_txn, ht_tbl_gateway_txn_def *pht_tbl_gateway_txn, int flag)
{
    ...
    if(flag == 1)
    {
        memcpy(tbl_err_flow.dest_pan	,     ptbl_alipay_wechat->pan,  sizeof(tbl_err_flow.dest_pan)-1);
        memcpy(tbl_err_flow.dest_ccy	,      ptbl_alipay_wechat->ccy,  sizeof(tbl_err_flow.dest_ccy)-1);
        memcpy(tbl_err_flow.dest_trans_at,   ptbl_alipay_wechat->trans_at,  sizeof(tbl_err_flow.dest_trans_at)-1);
    }
    else 
    {
...
*函数返回∶ 1
***************************************************/
static int in_cmp_flow(char *flag_result, tbl_alipay_wechat_def *ptbl_alipay_wechat, ht_tbl_host_txn_def *pht_tbl_host_txn, ht_tbl_gateway_txn_def *pht_tbl_gateway_txn)
{
    ...
    if(atof(ptbl_alipay_wechat->wx_fee) < 0)
    {
        sprintf(tbl_cmp_flow.trans_at2,"%.2f",  atof(ptbl_alipay_wechat->wx_fee)*-1);
    }
    else
    {
         memcpy(tbl_cmp_flow.trans_at2,ptbl_alipay_wechat->wx_fee,  sizeof(tbl_cmp_flow.trans_at2)-1);       
    }
    ...
    {
        /*退款交易，取退款金额*/
        memcpy(tbl_cmp_flow.trans_at, ptbl_alipay_wechat->trans_at3,  sizeof(tbl_cmp_flow.trans_at3)-1);
    }
    memcpy(tbl_cmp_flow.trans_at4,ptbl_alipay_wechat->wx_rate,  sizeof(tbl_cmp_flow.trans_at4)-1);
...
static update_account_tab(char *flag_result, tbl_alipay_wechat_def *ptbl_alipay_wechat, ht_tbl_host_txn_def *pht_tbl_host_txn, ht_tbl_gateway_txn_def *pht_tbl_gateway_txn)
{
	int iRet = 0;
    /*更新网关流水对账状态*/
    memcpy(pht_tbl_gateway_txn->flag_result, flag_result,  BT_FLAG_RESULT_LEN);
...
*函数返回∶ 0  -- 成功
            -1 -- 失败
***************************************************/
int Task_0700 ( int nBeginOffset, int nEndOffset )
{
    ...
    while (1)
    {
        ...
        if (iRet)
        {
            ...
            HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "DbsTblAlipayWechatTxn fetch sqlcode[%d]", iRet);
        ...
        if(memcmp(ht_tbl_gateway_txn.trans_tp, BT_TRANS_TP_REFUND, BT_TRANS_TP_LEN) == 0)
        {
            ...
            memcpy(tbl_alipay_wechat.plat_key, ht_tbl_gateway_txn.host_key, sizeof(tbl_alipay_wechat.plat_key)-1);
            memcpy(tbl_alipay_wechat.mcht_refund_order, ht_tbl_gateway_txn.plat_key+8, sizeof(tbl_alipay_wechat.mcht_refund_order)-1-8);
            ...
            RightTrim(tbl_alipay_wechat.mcht_refund_order);
            RightTrim(tbl_alipay_wechat.inter_brh_code);
            HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "plat_key[%s]mcht_refund_order[%s]", tbl_alipay_wechat.plat_key, tbl_alipay_wechat.mcht_refund_order);
            iRet = DbsTblAlipayWechatTxn(DBS_SELECT4, &tbl_alipay_wechat, nBeginOffset, nEndOffset);
            if (iRet)
            {
                if (iRet == DBS_NOTFOUND)
                {
					...
					if(!memcmp(ht_tbl_gateway_txn.sett_date, dbtbl_date_inf.stoday, BT_DATE_LEN))
                    {
                        ...
                        iRet = in_err_flow(BT_FLAG_RESULT_PART_ORLESS, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn, 1);
                        if (iRet)
                        {
                ...
                HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "DbsTblAlipayWechatTxn DBS_SELECT4 sqlcode[%d]", iRet);
                DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
                return -1;
            }
        }
        else
        {
            ...
            memset(&tbl_alipay_wechat, 0x00, sizeof(tbl_alipay_wechat));
            memcpy(tbl_alipay_wechat.plat_key, ht_tbl_gateway_txn.plat_key, sizeof(tbl_alipay_wechat.plat_key)-1);
            memcpy(tbl_alipay_wechat.inter_brh_code, ext_inter_brh_code, sizeof(tbl_alipay_wechat.inter_brh_code)-1);
            RightTrim(tbl_alipay_wechat.plat_key);
            HtLog(gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "plat_key[%s]", tbl_alipay_wechat.plat_key);
            iRet = DbsTblAlipayWechatTxn(DBS_SELECT3, &tbl_alipay_wechat, nBeginOffset, nEndOffset);
            if (iRet)
            {
                if (iRet == DBS_NOTFOUND)
                {
                    ...
                    if (iRet)
                    {
                        if (iRet == DBS_NOTFOUND)
                        {
                            ...
                            iRet =  in_err_flow(BT_FLAG_RESULT_GATEWAY, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn, 1);
                            if (iRet)
                            {
                                HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "in_err_flow err sqlcode[%d]", iRet);
                                DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
					...
					if(!memcmp(ht_tbl_gateway_txn.sett_date, dbtbl_date_inf.stoday, BT_DATE_LEN))
                    {
                        ...
                        memset(&ht_tbl_host_txn, 0, sizeof(ht_tbl_host_txn));
                        iRet = in_err_flow(BT_FLAG_RESULT_PART_ORLESS, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn, 2);
                        if (iRet)
                        {
                            HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "in_err_flow err sqlcode[%d]", iRet);
                            DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
                ...
                HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "DbsTblAlipayWechatTxn DBS_SELECT3 sqlcode[%d]", iRet);
                DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
                return -1;
            }
        }
        ...
        if(sFl_Flag_Amt[0] == 'Y')
        {
            if(abs(atof(ht_tbl_gateway_txn.trans_at) -atof(tbl_alipay_wechat.trans_at))<=0.000001)
            {
                iAmtTag = 1;
            }
            else
            {
                HtLog (gLogFile, HT_LOG_MODE_NORMAL, __FILE__, __LINE__, "Amt not match: gateway[%13.2f], Alipay[%13.2f]",
                                ht_tbl_gateway_txn.plat_key, atof(ht_tbl_gateway_txn.trans_at), atof(tbl_alipay_wechat.trans_at));
                iAmtTag = 0;
            }
        }
        ...
        if (iAmtTag == 1 && iPanTag == 1)
        {
            if(iCheckTag == 0)
            {
                ...
                memset(&ht_tbl_host_txn, 0 , sizeof(ht_tbl_host_txn));
                iRet = in_cmp_flow(BT_FLAG_RESULT_CMP, &tbl_alipay_wechat,  &ht_tbl_host_txn, &ht_tbl_gateway_txn);
                if (iRet != 0)
                {
        ...
        else
        {
			...
			if (iRet && iRet != DBS_NOTFOUND)
            {
                ...
                // 更新表HT_TBL_HOST_TXN、HT_TBL_GATEWAY_TXN、tbl_alipay_wechat
				iRet = update_account_tab(BT_FLAG_RESULT_NOTMATC, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn);
				if(iRet)
                {
                    HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "update HT_TBL_HOST_TXN、HT_TBL_GATEWAY_TXN、tbl_alipay_wechat err sqlcode[%d]", iRet);
                    DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
                    return -1;
                }
            ...
            iRet =  in_err_flow(BT_FLAG_RESULT_NOTMATC, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn, 1);
            if (iRet)
            {
                HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "in_err_flow err sqlcode[%d]", iRet);
                DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
        ...
        if (iRet)
        {
            if (iRet == DBS_NOTFOUND)
            {
                ...
                iRet =  in_err_flow(BT_FLAG_RESULT_CORENULL, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn, 2);
                if (iRet)
                {
        ...
        if(memcmp(ht_tbl_host_txn.trans_state, BT_TRANS_STATE_FAIL, BT_TRANS_STATE_LEN) == 0)
        {
			...
			if (iRet && iRet != DBS_NOTFOUND)
            {
                ...
                // 更新表HT_TBL_HOST_TXN、HT_TBL_GATEWAY_TXN、tbl_alipay_wechat
				iRet = update_account_tab(BT_FLAG_RESULT_COREFAIL, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn);
				if(iRet)
                {
                    HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "update HT_TBL_HOST_TXN、HT_TBL_GATEWAY_TXN、tbl_alipay_wechat err sqlcode[%d]", iRet);
                    DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
            ...
            iRet = in_err_flow(BT_FLAG_RESULT_COREFAIL, &tbl_alipay_wechat, &ht_tbl_host_txn, &ht_tbl_gateway_txn, 1);
            if (iRet)
            {
                HtLog (gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "in_err_flow err sqlcode[%d]", iRet);
                DbsTblGatewayTxn(DBS_CLOSE, &ht_tbl_gateway_txn, nBeginOffset, nEndOffset);
    ...
    HtLog( gLogFile, HT_LOG_MODE_ERROR, __FILE__, __LINE__, "Task_0700 Succ .");
    return 0;
}
/****************************** end of Task_0700.c ******************************/
